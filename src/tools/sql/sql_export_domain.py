...
type %s struct { %s''' % (className, linesep)
    ...
    for column in tableColumns:
      t = 'UnKnow'
      c = str(column['DATA_TYPE'], 'utf-8').lower()
      ...
      comment = str(column['COLUMN_COMMENT'], 'utf-8')
      columnType = str(column['COLUMN_TYPE'], 'utf-8')
...
func (%s) TableName() string {
    ...
    file_utils.write_file(
        _file_path + tableName['TABLE_NAME'] + '_po.log', classInfo + os.linesep, 'w')
...
''' % (classInfo, tab, columnName, comment, tab, columnName, tab, format_go_db_type_is_null(isNull, key, t),
    ...
    file_utils.write_file(
        _file_path + tableName['TABLE_NAME'] + '_bo.log', classInfo + os.linesep, 'w')
