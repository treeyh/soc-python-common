...
from loguru._logger import Core as _loguru_Core, Logger as _loguru_Logger
...
class Logger(_loguru_Logger):
  def __init__(self, **kwargs):
    super().__init__(core=_loguru_Core(), exception=None, depth=0, record=False, lazy=False, colors=False, raw=False, capture=True, patchers=[], extra={})
    self.configure(**kwargs)


_log_map: Dict[str, Logger] = {}
...
def get_logger(log_filt_path:str = '') -> Logger:
  global _log_map
  log = _log_map.get(log_filt_path, None)
  if log is not None:
    return log
  ...
  log_config = {
      "handlers": [
          {"sink": sys.stdout, "level": "INFO", "format": LOG_STD_FORMAT},
          # LOG_FILE_FORMAT   "serialize": True, 
          {"sink": log_filt_path, "level": "INFO", "format": log_formatter, "rotation": "1 hour", "retention": "3 day"},
      ],
  }
  log = Logger(**log_config)
  _log_map[log_filt_path] = log
  return log