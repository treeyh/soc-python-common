...
def get_user_name_by_github():
  ...
  try:
    ...
    if r.status_code != 200:
      print('请求github接口返回：'+str(r.status_code))
      return None
...
def run():
  ...
  try:
    ...
    if user_name == None:
      print('请求github接口获取用户名失败。')
      return

    file_path = format_path(path_format)

    url = upload_file(image_path, 'commit '+image_name, user_name, repo_name, file_path, image_name)
    md_url = url.replace('https://api.github.com/repos/'+user_name+'/'+repo_name +
                         '/contents', 'https://cdn.jsdelivr.net/gh/'+user_name+'/' + repo_name)
    print(md_url)
    pyperclip.copy('![desc]('+md_url+')')
    pyperclip.paste()
  except:
    print(traceback.format_exc())
    pyperclip.copy('上传图片失败')
  finally:
    close(image_path)
