...
from soc_common.utils import mysql_utils, str_utils, log_utils
from soc_common.model import ds_model, config_model
from soc_common.tools.export_db_model import export_db_model


class MysqlExportDbModel(export_db_model.ExportDbModel):
  def __init__(self):
  ...
  def export_model(self, conf: config_model.DataSourceConfig) -> ds_model.DataSourceModel:
    ...
    mysqlUtil = mysql_utils.get_mysql_utils(
  ...
  def get_db_version(self, conn: mysql_utils.MysqlUtils) -> str:
    """获取mysql版本
  ...
  def get_export_dbs(self, conn: mysql_utils.MysqlUtils, includes: List[str] = [], excludes: List[str] = []) -> List[ds_model.DbModel]:
    """获取需要导出结构的数据库列表
    ...
    for d in data:
      db_name = str_utils.format_bytes_to_str(d['db_name'])
      ...
      charset = str_utils.format_bytes_to_str(d['charset'])
      collation_name = str_utils.format_bytes_to_str(d['collation_name'])
  ...
  def fill_table_fields(self, conn: mysql_utils.MysqlUtils, dsModel: ds_model.DataSourceModel) -> ds_model.DataSourceModel:
    """获取数据库中的表信息
    ...
    for db in dsModel.dbs:
      ...
      for d in data:
        ...
        comment = str_utils.format_bytes_to_str(d['TABLE_COMMENT'])
        collation_name = str_utils.format_bytes_to_str(d['TABLE_COLLATION'])
        engine = str_utils.format_bytes_to_str(d['ENGINE'])
  ...
  def get_create_script(self, conn: mysql_utils.MysqlUtils, dbName: str, tableName: str) -> str:
    """获取表的创建脚本
  ...
  def get_fields(self, conn: mysql_utils.MysqlUtils, dbName: str, tableName: str) -> List[ds_model.FieldModel]:
    """获取数据表中列信息
    ...
    for d in data:
      ...
      ftype = str_utils.format_bytes_to_str(d['DATA_TYPE'])
      column_type = str_utils.format_bytes_to_str(d['COLUMN_TYPE'])
      length = str_utils.format_bytes_to_str(
          d['CHARACTER_MAXIMUM_LENGTH']) if d['CHARACTER_MAXIMUM_LENGTH'] != None else str_utils.format_bytes_to_str(d['NUMERIC_PRECISION'])
      scale = str_utils.format_bytes_to_str(d['NUMERIC_SCALE'])
      # on update CURRENT_TIMESTAMP
      default = str_utils.format_bytes_to_str(d['COLUMN_DEFAULT'])
      ext = str_utils.format_bytes_to_str(d['EXTRA'])
      ...
      comment = str_utils.format_bytes_to_str(d['COLUMN_COMMENT'])
      charset = str_utils.format_bytes_to_str(d['CHARACTER_SET_NAME'])
      collation_name = str_utils.format_bytes_to_str(d['COLLATION_NAME'])
      indexFlag = 0
      column_key = str_utils.format_bytes_to_str(d['COLUMN_KEY'])
      if column_key == 'PRI':
        indexFlag = 1
      elif column_key == 'UNI':
        indexFlag = 3
