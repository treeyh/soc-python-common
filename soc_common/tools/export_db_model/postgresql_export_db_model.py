...
class PostgresqlExportDbModel(export_db_model.ExportDbModel):
  def __init__(self):
    self.excludesDb = ['default_db', 'postgres']
  ...
  def export_model(self, conf: config_model.DataSourceConfig) -> ds_model.DataSourceModel:
    ...
    # dsm = self.fill_table_fields(postgresqlUtils, dsm)
  ...
  def fill_table_fields(self, conn: postgresql_utils.PostgresqlUtils, dsModel: ds_model.DataSourceModel) -> ds_model.DataSourceModel:
    """获取数据库中的表信息
  ...
  def get_fields(self, conn: postgresql_utils.PostgresqlUtils, dbName: str, tableName: str) -> List[ds_model.FieldModel]:
    """获取数据表中列信息
